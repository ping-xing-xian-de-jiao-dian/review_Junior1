...
- 构成
  ...
  1. 邮件客户端：读写Email消息、与服务器交互收发消息
  2. 邮件服务器：邮箱存储发给该用户的Email、消息队列存储等待发送的Email
  3. SMTP协议：邮件服务器之间传递消息的协议（客户端：发送消息的服务器；服务器：接收消息的服务器）
     - 使用持久性TCP进行Email消息的可靠传输
     - 传输过程：握手、消息的传输、关闭
     - 命令 / 响应交互模式（HTTP是请求 / 响应模式）
     - Email消息只能包含7位的ASCII码

  > #### SMTP与HTTP的对比
  >
  > 1. HTTP拉式（pull要使用作为浏览器要到SebServer把网页拉回本地）；SMTP退式（push发送方主动和接收方建立连接，将发送消息推送到接收方）
  > 2. HTTP每个对象封装在独立的响应消息中；SMTP多个对象在由多个部分构成的消息中发送
  > 3. 都是命令 / 响应交互模式
  > 4. 命令和状态代码都是ASCII码
...
- MIME：多媒体邮件扩展，通过在邮件头部增加额外的行以声明MIME的内容类型
  ...
  <img src="network_learning.assets/image-20211220192017543.png" alt="image-20211220192017543" style="zoom:70%;" />

- 邮件访问协议：从服务器获取邮件

  发送者服务器发给接收者服务器时用的是SMTP协议
...
- POP3
  ...
  2. 事务阶段
     ...
     - Dele：删除消息
     - Quit
  3. 下载并删除模式、下载并保持模式：不同客户端是否能重读该邮件
  4. **POP3是无状态的**

- IMAP

  所有消息同意保存在一个地方：服务器；允许利用文件夹组织消息；有状态
